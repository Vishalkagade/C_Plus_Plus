...
void ipb::serialization::sifts::ConvertDataset(
    ...
    if (!std::filesystem::exists(img_path.parent_path().parent_path() /
                                 "bin")) {
        std::filesystem::create_directories(
                img_path.parent_path().parent_path() / "bin");
    }
    for (const auto& p :
         std::filesystem::directory_iterator(img_path.parent_path())) {
...
std::vector<cv::Mat> ipb::serialization::sifts::LoadDataset(
    ...
    for (const auto& p :
        ...
        vec_out.push_back(ipb::serialization::Deserialize(p.path()));
    }
